...
def get_activities(
    ...
    if difficulty:
        if difficulty == DIFFICULTY_UNIVERSAL:
...
def get_available_difficulties() -> List[str]:
    """Get a list of all difficulty levels that are used in activities"""
    # Get all distinct difficulty values (excluding null/missing)
    difficulties = activities_collection.distinct("difficulty", {"difficulty": {"$exists": True, "$ne": None}})
